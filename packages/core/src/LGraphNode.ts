...
export type InputSlotLayout = {
    name: string;
    type: SlotType;
    options?: Partial<INodeInputSlot>;
};
export type OutputSlotLayout = {
    name: string;
    type: SlotType;
...
export type LGraphNodeConstructorFactory<T> = new (
    title?: string,
    ...restArgs: any[]
) => T;
...
export default class LGraphNode {
    ...
    constructor(title?: string) {
        ...
        if (LiteGraph.use_uuids) this.id = uuidv4();
    ...
    /**
     * FIXME: 我不太理解为什么 subgraph 需要挂在 Node 类上？这里设计的不太好
     */
    ...
    last_serialization?: SerializedLGraphNode | null = null;
    ...
    events = new EventEmitter<{
        ...
        changeMode: (mode: NodeMode, before: NodeMode | undefined) => void;
    }>({
    ...
    configure(info: SerializedLGraphNode): void {
        ...
        if (!info.title) {
            this.title =
                getStaticPropertyOnInstance<string>(this, "title") ||
                this.title;
        }
    ...
    clone(cloneData: LGraphNodeCloneData = { forNode: {} }): LGraphNode | null {
        if (!this.type) throw new Error("Type not defined");
        ...
        let data = LiteGraph.cloneObject<SerializedLGraphNode<LGraphNode>>(
            this.serialize(),
        );
    ...
    *iterateParentSubgraphNodes(): Iterable<Subgraph> {
    ...
    getInputSlotProperty(slot: SlotIndex, name: string): any {
        ...
        let input_info = this.inputs[slot];
        if (!input_info) {
            return;
        }
    ...
    setInputSlotProperty(slot: SlotIndex, name: string, value: any) {
        ...
        let prev_value = input_info.properties[name];
    ...
    setOutputSlotProperty(slot: SlotIndex, name: string, value: any) {
        ...
        let prev_value = output_info.properties[name];
        ...
        if (this.outputs[slot].links) {
            for (let i = this.outputs[slot].links.length - 1; i >= 0; i--) {
                ...
                if (link) {
                    ...
                    if (outputNode) {
                        const inputSlot = outputNode.getInputInfo(
                            link.target_slot,
                        );
                        if (
                            inputSlot &&
    ...
    *iterateInputInfo(): Iterable<INodeInputSlot> {
    ...
    getInputDataType(slot: number): SlotType | null {
        ...
        if (output_info && output_info.type != BuiltInSlotType.ACTION) {
     ...
     */
    getInputDataByName<T = any>(
        slot_name: string,
        force_update?: boolean,
    ): T | null | undefined {
        let slot = this.findInputSlotIndexByName(slot_name);
        if (slot == -1) {
            return null;
        }
    ...
    getInputLink(slot: SlotIndex): LLink | null {
        ...
        if (slot < this.inputs.length) {
            ...
            if (slot_info.link) return this.graph?.links[slot_info.link];
        }
        return null;
    }
    ...
    getInputNode(slot: SlotIndex): LGraphNode | null {
        ...
        if (slot < this.inputs.length) {
            ...
            let origin_node = this.graph?.getNodeById(link.origin_id);
            if (origin_node) {
                return origin_node;
            }
        }
     ...
     * @return {LLink} object or null
     */
    getOutputSlotConnectedTo(
        slot: SlotIndex,
    ): INodeOutputSlot | null | undefined {
        if (!this.outputs || !this.graph) {
            return null;
        }
    ...
    *iterateOutputInfo(): Iterable<INodeOutputSlot> {
    ...
    getOutputLinks(slot: SlotIndex): LLink[] {
        ...
        if (slot >= 0 && slot < this.outputs.length) {
            let slot_info = this.outputs[slot];
            if (slot_info.links) {
                let links: LLink[] = [];
    ...
    *iterateAllLinks(): Iterable<LLink> {
    ...
    addOnTriggerInput(): SlotIndex {
        let trigS = this.findInputSlotIndexByName("onTrigger");
        if (trigS == -1) {
            //!trigS ||
            this.addInput("onTrigger", BuiltInSlotType.EVENT, {
                optional: true,
                nameLocked: true,
            });
    ...
    addOnExecutedOutput(): SlotIndex {
        let trigS = this.findOutputSlotIndexByName("onExecuted");
        if (trigS == -1) {
            //!trigS ||
            this.addOutput("onExecuted", BuiltInSlotType.ACTION, {
                optional: true,
                nameLocked: true,
            });
    ...
    changeMode(modeTo: NodeMode): boolean {
        ...
        this.widgets?.forEach((w) => {
            if (this.mode) w.onNodeModeChange?.(this, this.mode);
        });
    ...
    trigger(action: string, param: any, options?: LActionOptions): void {
        ...
        for (let i = 0; i < this.outputs.length; ++i) {
            ...
            if (
                !output ||
                output.type !== BuiltInSlotType.EVENT ||
    ...
    triggerSlot(
        ...
        for (let k = 0; k < links.length; ++k) {
            ...
            let linkInfo = this.graph?.links[links[k]];
            if (!linkInfo) {
                //not connected
                continue;
            }
            linkInfo._last_time = LiteGraph.getTime();
            let node = this.graph?.getNodeById(linkInfo.target_id);
            if (!node) {
                //node not found?
                continue;
            }
    ...
    moveOutput(slot: number, newSlot: number): void {
        ...
        if (otherOutput.links) {
            for (const linkID of otherOutput.links) {
                ...
                if (otherLink) otherLink.origin_slot = slot;
            }
        }
    ...
    moveInput(slot: number, newSlot: number): void {
        ...
        if (otherInput.link != null) {
            ...
            if (otherLink) otherLink.target_slot = slot;
        }
    ...
    computeSize(out: Vector2 = [0, 0]): Vector2 {
        const overrideSize = getStaticPropertyOnInstance<Vector2>(
            this,
            "overrideSize",
        );
        if (overrideSize) {
        ...
        if (this.widgets && this.widgets.length) {
            for (const widget of this.widgets) {
                size[0] = Math.max(
                    size[0],
                    widget.width || LiteGraph.NODE_WIDTH * 1.5,
                );
            }
        }
        ...
        if (
            (this.constructor as any).min_height &&
    ...
    getPropertyInfo(property: string): IPropertyInfo {
        let info: IPropertyInfo | null = null;
        ...
        if (
            (this.constructor as any).widgets_info &&
            (this.constructor as any).widgets_info[property]
        )
    ...
    addWidget<T extends AllBuiltinWidget>(
        ...
        name: string,
        value: T["value"],
        callback?: WidgetCallback<T> | string | null,
        options?: T["options"],
    ): T {
        if (!this.widgets) {
            this.widgets = [];
    ...
    getSlotInPosition(x: number, y: number): SlotInPosition | null {
        //search for inputs
        let link_pos: Vector2 = [0, 0];
        if (this.inputs) {
            for (let i = 0, l = this.inputs.length; i < l; ++i) {
                let input = this.inputs[i];
     ...
     */
    findInputSlotIndexByName(
        name?: string,
        onlyFree: boolean = false,
        typesNotAccepted?: SlotType[],
    ): number {
        if (!this.inputs) {
            return -1;
        }
        for (let i = 0, l = this.inputs.length; i < l; ++i) {
            ...
            if (
                typesNotAccepted &&
                typesNotAccepted.includes(this.inputs[i].type)
            ) {
                continue;
            }
    ...
    findOutputSlotIndexByName(
        ...
        for (let i = 0, l = this.outputs.length; i < l; ++i) {
            if (
                onlyFree &&
                this.outputs[i].links &&
                this.outputs[i].links != null
            ) {
                continue;
            }
            if (
                typesNotAccepted &&
                typesNotAccepted.includes(this.outputs[i].type)
            ) {
                continue;
            }
    ...
    findOutputSlotByName(
        name?: string,
        onlyFree: boolean = false,
        typesNotAccepted?: SlotType[],
    ): INodeOutputSlot | null {
        if (!this.outputs) {
            return null;
        }
        for (let i = 0, l = this.outputs.length; i < l; ++i) {
     ...
     */
    findInputSlotIndexByType(
        type: SlotType,
        preferFreeSlot: boolean = false,
        doNotUseOccupied: boolean = false,
    ): number {
        return this.findSlotByType<INodeInputSlot>(
            true,
            type,
            false,
            preferFreeSlot,
     ...
     */
    findOutputSlotIndexByType(
        type: SlotType,
        preferFreeSlot: boolean = false,
        doNotUseOccupied: boolean = false,
    ): number {
        return this.findSlotByType<INodeOutputSlot>(
            false,
            type,
            false,
            preferFreeSlot,
     ...
     */
    private findSlotByType<T extends INodeSlot>(
        input: boolean,
        type: SlotType,
        returnObj: boolean,
        preferFreeSlot: boolean = false,
        ...
        if (type == "" || type == "*") type = BuiltInSlotType.DEFAULT;
        for (let i = 0, l = aSlots.length; i < l; ++i) {
            let tFound = false;
            let aSource: SlotType[] = (type + "").toLowerCase().split(",");
            let aDests = (
    ...
    connectByTypeInput(
        ...
        if (targetNode && targetNode.constructor === Number) {
            ...
            throw new Error("targetNode is not a valid LGraphNode");
        }
        ...
        let targetSlot = targetNode.findInputSlotIndexByType(
            sourceSlotType,
            true,
        );
        if (targetSlot >= 0 && targetSlot !== null) {
            if (LiteGraph.debug)
                console.debug(
                    "CONNbyTYPE type " + targetSlotType + " for " + targetSlot,
                );
            return this.connect(slot, targetNode, targetSlot);
        } else {
            if (LiteGraph.debug)
                console.log(
                    "type " + targetSlotType + " not found or not free?",
                );
            if (
                ...
                if (LiteGraph.debug)
                    console.debug(
                        "connect WILL CREATE THE onTrigger " +
                            targetSlotType +
                            " to " +
                            targetNode,
                    );
            ...
            if (
                ...
                (targetSlotType == BuiltInSlotType.DEFAULT ||
                    targetSlotType == "*" ||
                    targetSlotType == "")
            ) {
                let targetSlot = targetNode.findInputSlotIndexByName(
                    undefined,
                    true,
    ...
    connectByTypeOutput(
        ...
        if (sourceNode && sourceNode.constructor === Number) {
            ...
            throw new Error("sourceNode is not a LGraphNode");
        }
        ...
        const sourceSlot = sourceNode.findOutputSlotIndexByType(
            targetSlotType,
            true,
        );
        if (sourceSlot >= 0 && sourceSlot !== null) {
            ...
            // connect to the first general output slot if not found a specific type and
            if (opts.generalTypeInCase) {
                let sourceSlot = sourceNode.findOutputSlotIndexByType(
                    BuiltInSlotType.DEFAULT,
                    true,
                    true,
                );
            ...
            if (
                ...
                if (LiteGraph.do_add_triggers_slots) {
                    let sourceSlot = sourceNode.addOnExecutedOutput();
                    return sourceNode.connect(sourceSlot, this, slot);
                }
            }
            ...
            if (
                ...
                (sourceSlotType == BuiltInSlotType.DEFAULT ||
                    sourceSlotType == "*" ||
                    sourceSlotType == "")
            ) {
                let sourceSlot = sourceNode.findOutputSlotIndexByName(
                    undefined,
                    true,
                    [BuiltInSlotType.EVENT, BuiltInSlotType.ACTION],
                );
                if (sourceSlot >= 0) {
                    return sourceNode.connect(sourceSlot, this, slot);
                }
            ...
            console.error(
                "type OUT! " + sourceSlotType + " not found or not free?",
            );
            return null;
        }
    }
     ...
     * @return {LLink} the linkInfo is created, otherwise null
     */
    connect(
        ...
        let targetSlotIdx = typeof targetSlot === "number" ? targetSlot : 0;
        ...
        if (targetNode && targetNode.constructor === Number) {
            // targetNode = this.graph?.getNodeById(targetNode);
            throw new Error(
                "Connect: Error, target node is a number, not a node",
            );
        }
        if (!targetNode) {
        ...
        if (typeof targetSlot === "string") {
            targetSlotIdx = targetNode.findInputSlotIndexByName(targetSlot);
            if (targetSlotIdx == -1) {
                if (LiteGraph.debug) {
                    console.error(
            ...
            if (LiteGraph.do_add_triggers_slots) {
                ...
                if (LiteGraph.debug) {
                    console.error(
                        "Connect: Error, can't connect event target slot",
                    );
                }
                return null; // -- break --
            }
        ...
        let input = targetNode.inputs[targetSlot];
        let linkInfo: null | LLink = null;
        let output = this.outputs[slot];
        ...
        if (typeof targetSlot === "number" && targetNode.onBeforeConnectInput) {
        ...
        if (targetNode.onConnectInput && typeof targetSlot === "number") {
            if (
                targetNode.onConnectInput(
                    targetSlot,
                    output.type,
                    output,
                    this,
        ...
        if (this.onConnectOutput && typeof targetSlot === "number") {
            // callback
            if (
            ...
            switch (output.type) {
                case BuiltInSlotType.EVENT:
                    if (!LiteGraph.allow_multi_output_for_events) {
                        this.graph?.beforeChange();
                        this.disconnectOutput(slot, null, {
                            doProcessChange: false,
                        }); // Input(targetSlot, {doProcessChange: false});
                        changed = true;
                    }
                    break;
                default:
        ...
        linkInfo = new LLink(
            ...
            slot,
            targetNode.id,
            // @ts-ignore FIXME 需要改上面的逻辑增加中间值
            targetSlot,
        );

        if (this.graph?.links[linkInfo.id]) {
            console.error(
                "Link already exists in graph!",
                linkInfo.id,
                linkInfo,
                this.graph?.links[linkInfo.id],
            );
        }
        ...
        if (targetNode) {
            ...
            for (let i = 0, l = output.links.length; i < l; i++) {
                ...
                if (link_info.target_id == targetNode.id) {
                    ...
                    let input = targetNode.inputs[link_info.target_slot];
    ...
    disconnectInput(
        slot: SlotNameOrIndex,
        options: { doProcessChange?: boolean } = {},
    ): boolean {
        //seek for the output slot
        if (typeof slot === "string") {
        ...
        if (link_id != null) {
            ...
            let link_info = this.graph?.links[link_id];
            if (link_info) {
                let targetNode = this.graph?.getNodeById(link_info.origin_id);
                if (!targetNode) {
                    return false;
                }

                let output = targetNode.outputs[link_info.origin_slot];
                if (!output || !output.links || output.links.length == 0) {
                    return false;
                }
    ...
    isShowingTitle(mouseOver: boolean): boolean {
        if (
            this.titleMode == TitleMode.TRANSPARENT_TITLE ||
            this.titleMode == TitleMode.NO_TITLE
        ) {
            return false;
    ...
    getOptionalSlots(): OptionalSlots | null {
        return getStaticPropertyOnInstance<OptionalSlots>(
            this,
            "optionalSlots",
        );
    }
